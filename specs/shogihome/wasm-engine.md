...
深さ 2 以降はモジュール内部のタイマーから 1 反復ずつ進め、反復の切れ目で `stop` を
受け付けられる。思考を進めるためのメソッドは公開インターフェースに含めない。

C++ 側の内部駆動関数は `engines/core/shim.js` (`--pre-js`) が呼び、ABI が定める
公開インターフェースは同シムが組み立てる。
